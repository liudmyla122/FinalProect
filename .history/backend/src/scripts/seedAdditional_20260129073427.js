/**
 * Добавляет тестовых пользователей и посты БЕЗ удаления существующих данных.
 * Ваш аккаунт и ваши посты остаются. В ленте появятся посты от других пользователей.
 *
 * Запуск: cd backend && npm run seed:add
 */
...
// Запасной вариант: SVG (цветной фон + текст), если нет сети.
...
// Загрузка реальных фото из открытого сервиса (picsum.photos) и сохранение в формате base64 для БД.
// Требуется Node 18+ (встроенный fetch). Иначе используются запасные картинки.
...
/** Возвращает массив из 8 картинок: реальные фото с picsum.photos или запасные SVG. */
async function getPostImages() {
  ...
  if (realCount > 0) console.log(`   ✓ Загружено фото: ${realCount}/8 (остальные — запасные)`);
  else console.log('   ⚠ Используются запасные картинки (нет сети или ошибка).');
  return images;
}

/** Аватары пользователей: 200x200, сохраняются в БД в поле user.avatar. */
...
const testUsers = [
  { email: 'sashaa@example.com', username: 'sashaa', fullName: 'Sasha Anderson', password: 'password123', avatar: '', bio: 'Photographer and traveler 🌍' },
  { email: 'igor@example.com', username: 'igor', fullName: 'Igor Petrov', password: 'password123', avatar: '', bio: 'Developer and designer 💻' },
  { email: 'marija@example.com', username: 'marija', fullName: 'Marija Novak', password: 'password123', avatar: '', bio: 'Artist and creator 🎨' },
  { email: 'alex@example.com', username: 'alex', fullName: 'Alex Johnson', password: 'password123', avatar: '', bio: 'Fitness enthusiast 💪' },
  { email: 'lisa@example.com', username: 'lisa', fullName: 'Lisa Brown', password: 'password123', avatar: '', bio: 'Food blogger 🍕' },
  { email: 'david@example.com', username: 'david', fullName: 'David Wilson', password: 'password123', avatar: '', bio: 'Musician and producer 🎵' },
  { email: 'emma@example.com', username: 'emma', fullName: 'Emma Davis', password: 'password123', avatar: '', bio: 'Writer and poet 📝' },
  { email: 'mike@example.com', username: 'mike', fullName: 'Mike Taylor', password: 'password123', avatar: '', bio: 'Adventure seeker 🏔️' },
];

function buildTestPosts(users, images) {
  const img = (i) => (images && images[i]) || FALLBACK_IMAGES[i];
  return [
    { user: users[0]._id, image: img(0), images: [img(0)], isVideo: false, caption: 'Beautiful sunset today! 🌅 #sunset #nature', likes: [users[1]._id, users[2]._id], comments: [{ user: users[1]._id, text: 'Amazing shot! 🔥' }] },
    { user: users[1]._id, image: img(1), images: [img(1)], isVideo: false, caption: 'Working on a new project. Stay tuned! 💻 #coding', likes: [users[0]._id], comments: [] },
    { user: users[2]._id, image: img(2), images: [img(2)], isVideo: false, caption: 'New artwork in progress 🎨 #art #creative', likes: [users[0]._id, users[1]._id], comments: [{ user: users[0]._id, text: 'Stunning! ✨' }] },
    { user: users[3]._id, image: img(3), images: [img(3)], isVideo: false, caption: 'Morning workout complete! 💪 #fitness', likes: [], comments: [] },
    { user: users[4]._id, image: img(4), images: [img(4)], isVideo: false, caption: 'Delicious homemade pasta 🍝 #food', likes: [users[0]._id], comments: [] },
    { user: users[5]._id, image: img(5), images: [img(5)], isVideo: false, caption: 'New track dropping soon! 🎵 #music', likes: [], comments: [] },
    { user: users[6]._id, image: img(6), images: [img(6)], isVideo: false, caption: 'Words have power ✍️ #writing #poetry', likes: [users[2]._id], comments: [] },
    { user: users[7]._id, image: img(7), images: [img(7)], isVideo: false, caption: 'Mountain adventure! 🏔️ #adventure', likes: [users[0]._id, users[1]._id], comments: [] },
  ];
}
...
const seedAdditional = async () => {
  try {
    await connectDB();
    console.log('📥 Добавление тестовых пользователей и постов (без удаления существующих данных)...\n');
    ...
    // Связи following/followers — считаем в памяти и сохраняем каждого пользователя один раз,
    // иначе Mongoose VersionError при повторном сохранении того же документа.
    ...
    // Аватары пользователей: загружаем фото и сохраняем в БД (user.avatar, user.avatarType).
    ...
    // Обновляем картинки у уже существующих постов тестовых пользователей (реальные фото или запасные)
    ...
    console.log('   Пароль тестовых аккаунтов: password123 (например sashaa, igor, marija).');
    process.exit(0);
  } catch (error) {
