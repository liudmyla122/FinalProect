/**
 * Скрипт добавляет 5 зарегистрированных пользователей с аватарками и постами,
 * создаёт посты у владельца (если их нет) и добавляет комментарии под постами владельца.
 *
 * Запуск:
 *   node src/scripts/seedCommenters.js
 *   node src/scripts/seedCommenters.js Liudmyla
 *
...
// Фотографии для аватарок (URL — разные лица для каждого пользователя)
const AVATAR_PHOTOS = [
...
// Минимальная картинка для постов (маленький PNG, отображается как пост)
const PLACEHOLDER_POST_IMAGE =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==';

// Пять пользователей: аккаунты + аватарки с фотографиями
const COMMENTERS = [
  {
...
// Комментарии под постами владельца
const COMMENT_TEXTS = [
...
// Посты для владельца (если у него нет постов)
...
// По одному посту для каждого комментатора (подпись к посту)
...
async function seedCommenters() {
  try {
    ...
    if (!owner) {
      ...
      if (owner) {
        console.log(
          '\n⚠️  По ключу "' + ownerKey + '" пользователь не найден. Используется пользователь с постами:',
          owner.username,
          '\n'
        );
      } else {
    ...
    if (myPosts.length === 0) {
      ...
      for (let i = 0; i < OWNER_POST_CAPTIONS.length; i++) {
        ...
        console.log('   ✓ Пост создан:', OWNER_POST_CAPTIONS[i].slice(0, 40) + '...');
      }
