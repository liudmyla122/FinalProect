...
const createPost = async (req, res) => {
  try {
    ...
    console.log('📝 Создание поста:', {
      ...
      postData_profileId: postData.profileId,
      postData_user: postData.user,
    })
...
const getMyPosts = async (req, res) => {
  try {
    ...
    console.time('🔍 getMyPosts: Database query')
    let posts = []
    try {
      posts = await Post.find(filter).sort({ createdAt: -1 }).lean()
      console.timeEnd('🔍 getMyPosts: Database query')
    ...
    console.time('🔍 getMyPosts: Formatting posts')
    ...
    console.timeEnd('🔍 getMyPosts: Formatting posts')
...
const getFeed = async (req, res) => {
  try {
    ...
    console.time('🔍 getFeed: Total time')
    console.time('🔍 getFeed: Database query')
    ...
    console.timeEnd('🔍 getFeed: Database query')
    console.log(
      `📊 Загружено ${posts.length} постов, skip: ${skip}, limit: ${safeLimit}`
    )

    const userId = req.user?._id
    console.time('🔍 getFeed: Formatting posts')
    const formatted = posts.map((post) => {
      const liked =
        userId &&
        Array.isArray(post.likes) &&
      ...
      return {
        id: post._id,
        profileId: post.profileId != null ? String(post.profileId) : null,
        image: post.image?.substring(0, 100) + '...', // Log truncated for performance
        images:
          post.images && post.images.length > 0 ? post.images : [post.image],
        isVideo: post.isVideo || false,
        title: post.title != null ? String(post.title) : '',
    ...
    console.timeEnd('🔍 getFeed: Formatting posts')
    console.timeEnd('🔍 getFeed: Total time')
