...
const login = async (req, res) => {
  try {
    ...
    const user = await User.findOne(
      isEmail ? { email: loginInput.toLowerCase() } : { username: loginInput },
    )
...
const getCurrentUser = async (req, res) => {
  try {
    ...
    // Оптимизация: убрана тяжелая проверка размера JSON для логов
    // try {
...
const updateAvatar = async (req, res) => {
  try {
    const { avatar, avatarType: explicitAvatarType, profileId } = req.body || {}
    ...
    if (!avatar || typeof avatar !== 'string') {
      ...
      return res
        .status(400)
        .json({ message: 'Avatar (base64 string) is required' })
    }
    ...
    if (avatarSizeMB > MAX_AVATAR_SIZE_MB) {
      console.warn('⚠️ Аватар слишком большой:', avatarSizeMB.toFixed(2), 'MB')
      return res.status(400).json({
        message: `Avatar is too large (${avatarSizeMB.toFixed(
          2,
        )} MB). Maximum size is ${MAX_AVATAR_SIZE_MB} MB.`,
    ...
    if (avatarSizeMB > 15) {
      console.warn(
        '⚠️ ВНИМАНИЕ: Аватар превышает лимит MongoDB (16MB). Может потребоваться GridFS или внешнее хранилище.',
      )
    }

    let avatarType = 'image'
    if (explicitAvatarType === 'video' || explicitAvatarType === 'image') {
      avatarType = explicitAvatarType
    } else {
      avatarType = avatar.startsWith('data:video/') ? 'video' : 'image'
    }

    let targetProfile = null

    // Logic to update specific profile
    if (profileId) {
      ...
      if (profileIndex >= 0) {
        console.log(
          `✅ Обновляем аватар для вторичного профиля: ${profileId} (index ${profileIndex})`,
        )
        // Direct modification of array element
        user.profiles[profileIndex].avatar = avatar
        user.profiles[profileIndex].avatarType = avatarType
        user.markModified('profiles') // Important for Mongoose array updates
        targetProfile = user.profiles[profileIndex]
      } else if (String(profileId) === String(user._id)) {
        console.log('✅ Обновляем аватар для основного профиля (по ID)')
        user.avatar = avatar
        user.avatarType = avatarType
      } else {
        console.warn(
          `⚠️ ProfileId ${profileId} не найден, обновляем основной аватар как fallback`,
        )
        user.avatar = avatar
        user.avatarType = avatarType
      }
    } else {
      console.log('✅ Обновляем аватар для основного профиля (без profileId)')
      user.avatar = avatar
      user.avatarType = avatarType
    }
    ...
    try {
      ...
      if (profileId && targetProfile) {
        const savedProfile = savedUser.profiles.find(
          (p) => String(p.id) === String(profileId),
        )
        avatarSaved =
          savedProfile && savedProfile.avatar && savedProfile.avatar.length > 0
      } else {
        avatarSaved =
          savedUser && savedUser.avatar && savedUser.avatar.length > 0
      }
      ...
      if (!avatarSaved) {
        console.error(
          '❌ КРИТИЧЕСКАЯ ОШИБКА: Аватар не сохранился в базу данных!',
        )
...
const updateProfiles = async (req, res) => {
  try {
    const { profiles, organization } = req.body || {}
    ...
    const cleanedProfiles = profiles.map((profile) => {
      ...
      // Safe Merge: If avatar is empty in request, try to preserve existing avatar from DB
      let finalAvatar = avatarVal
      let finalAvatarType = type

      if (!finalAvatar && user.profiles) {
        const existingProfile = user.profiles.find(
          (p) => p && String(p.id) === String(profile.id || ''),
        )
        if (existingProfile && existingProfile.avatar) {
          // console.log(`Preserving avatar for profile ${profile.id}`);
          finalAvatar = existingProfile.avatar
          finalAvatarType = existingProfile.avatarType || 'image'
        }
      }
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(200).json({
        message:
          'If an account with that email or username exists, we have sent a password reset link.',
      })
    }
