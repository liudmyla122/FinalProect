...
const updateAvatar = async (req, res) => {
  try {
    ...
    const MAX_AVATAR_SIZE_MB = 1024 * 1024 // 1TB (практически безлимит)
    ...
    // Логика обновления конкретного профиля
    if (profileId) {
      // Ищем профиль в массиве profiles
      ...
      if (profileIndex >= 0) {
        // Обновляем вторичный профиль
        console.log(`✅ Обновляем аватар для вторичного профиля: ${profileId}`)
        user.profiles[profileIndex].avatar = avatar
        user.profiles[profileIndex].avatarType = avatarType
        user.markModified('profiles') // Важно для Mongoose!
        targetProfile = user.profiles[profileIndex]
      } else if (
        String(profileId) === String(user._id) ||
        profileId === 'main'
      ) {
        ...
        console.warn(
          `⚠️ Профиль с ID ${profileId} не найден, обновляем основной профиль как запасной вариант`,
        )
        user.avatar = avatar
        user.avatarType = avatarType
        targetProfile = user
      ...
      // Если profileId не передан, обновляем основной
      console.log(`✅ Обновляем аватар для основного профиля (по умолчанию)`)
      user.avatar = avatar
      user.avatarType = avatarType
      targetProfile = user
    }
    ...
    try {
      ...
      // Проверяем, где именно мы обновляли
      let avatarSaved = false
      if (targetProfile === user || !profileId) {
        avatarSaved =
