...
const createPost = async (req, res) => {
  try {
    ...
    const postData = {
      ...
      image: imageArray[0],
      // OPTIMIZATION: Do not duplicate data in 'images' if there is only one image.
      // This saves ~50% of document size for single-file posts (especially videos).
      images: imageArray.length > 1 ? imageArray : [],
    ...
    // Calculate REAL document size (approximate)
    // If images.length > 1, we still have duplication of the first image (image + images[0]),
    // but for single image/video (common case for large files), duplication is removed.
    let realTotalSize = postData.image.length
    if (postData.images.length > 0) {
      realTotalSize += postData.images.reduce((sum, img) => sum + img.length, 0)
    }

    const totalSizeMB = (realTotalSize * 3) / 4 / 1024 / 1024

    const MAX_DOCUMENT_SIZE_MB = 15 // Mongoose limit is 16MB
