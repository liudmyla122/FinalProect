...
const getMyPosts = async (req, res) => {
  try {
    ...
    if (profileId) {
      console.log('📥 Применяем фильтр profileId:', profileId)
      filter.profileId = profileId
    } else {
      console.log('⚠️ profileId не указан, показываем все посты пользователя')
    }
...
const getFeed = async (req, res) => {
  try {
    ...
    console.log(`📊 Загружено ${posts.length} постов, skip: ${skip}, limit: ${safeLimit}`)
