  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        // Проверяем активный профиль в localStorage
        try {
          ...
          if (activeId && profilesRaw) {
            ...
            if (activeProfile) {
              ...
              effectiveUser = {
                ...activeProfile,
                id: activeProfile.id, // ID профиля
                dbId: currentUser.id, // Ссылка на основной ID БД
              ...
              console.log(
                '✅ Установлен вторичный профиль:',
                effectiveUser.username,
              )
            } else {
              console.log(
                '⚠️ Активный профиль не найден в списке, используем основной',
              )
            }
          } else if (
            currentUser.avatar &&
            ...
            // Обработка аватара основного пользователя
            effectiveUser.avatar = currentUser.avatar
            effectiveUser.avatarType =
              currentUser.avatarType ||
              (currentUser.avatar.startsWith('data:video/') ? 'video' : 'image')
          }
        } catch (e) {
          console.error('Ошибка при определении активного профиля:', e)
        }

        setUser(effectiveUser)

        // Обновляем список профилей в localStorage
    ...
    <div className="home-container">
      <div className="home-layout">
        ...
        <header className="home-mobile-header">
          <div className="home-mobile-header-logo">ICHGRAM</div>
        ...
        <nav className="home-mobile-nav">
          <Link to="/" className="home-mobile-nav-item">
            <svg
              className="home-mobile-nav-icon"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
          ...
          <Link to="/explore" className="home-mobile-nav-item">
            <svg
              className="home-mobile-nav-icon"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <Link to="/profile" className="home-mobile-nav-item">
            {user?.avatar ? (
              user.avatarType === 'video' ||
              (typeof user.avatar === 'string' &&
                user.avatar.startsWith('data:video/')) ? (
                <video
                  src={user.avatar}
                  className="home-mobile-profile-img"
                  autoPlay
                  loop
                  muted
                  playsInline
