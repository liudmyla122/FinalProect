...
// Статические превью для уведомлений (они не зависят от реальных постов)
...
// Только число (без слова «likes»): 1, 1.2k и т.д.
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        // Загружаем аватар из базы данных, если он есть
        ...
        // Синхронизируем профили из backend в localStorage,
        // чтобы они не терялись при перезаходе в приложение
        if (currentUser && Array.isArray(currentUser.profiles)) {
          ...
          if (serverProfiles.length > 0) {
            ...
            // Если активный профиль ещё не выбран, выбираем первый
            ...
            if (activeId) {
              ...
              // Загружаем аватар из отдельного хранилища, если он есть
              try {
                ...
                if (avatarFromStorage) {
                  ...
                  // Если аватар есть в базе данных, используем его
              ...
              setUser({ ...activeProfile, dbId: currentUser.id }); // dbId — id пользователя в БД для сравнения с автором поста
            }
          } else {
        ...
        // Редирект на логин только при 401 (недействительная сессия). Иначе пользователь остаётся на ленте.
  ...
  // Загружаем фид для главной страницы (все посты всех пользователей из БД)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Debounce поиска - ждем 300ms после последнего ввода
  ...
  return (
    <div className="home-container">
      <div className="home-layout">
        ...
        {/* Затемнение основного контента при открытии панели поиска или уведомлений */}
        ...
        {showSearchPanel && (
          <div className="home-search-panel">
            {/* Полупрозрачные кружочки для просмотра основной страницы */}
        ...
        <main className={`home-main ${showSearchPanel || showNotificationsPanel ? 'home-main-dimmed' : ''}`}>
          ...
          {/* Секция "You've seen all the updates" */}
