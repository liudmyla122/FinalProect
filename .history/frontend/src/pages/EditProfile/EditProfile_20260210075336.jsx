...
const EditProfile = () => {
  ...
  const handleSave = async (e) => {
    ...
    // Determine activeId early for updateAvatar
    let activeId = (user && user.id) || localStorage.getItem('activeProfileId')
    if (!activeId) {
    ...
    if (avatar && typeof avatar === 'string' && avatar.trim().length > 0) {
      try {
        ...
        const avatarResponse = await authAPI.updateAvatar(
          avatar,
          avatarType,
          activeId,
        )
    ...
    try {
      ...
      // activeId is already calculated at the top
      ...
      try {
        ...
        const profilesForServer = [...serverProfiles]

        // 1. Merge all local profiles into the server list
        for (const localP of newList) {
          const idx = profilesForServer.findIndex(
            (s) => s && String(s.id) === String(localP.id),
          )
          if (idx >= 0) {
            // Update existing
            profilesForServer[idx] = { ...profilesForServer[idx], ...localP }
          } else {
            // Add new local profile to server list
            profilesForServer.push(localP)
          }
        }

        // 2. Explicitly ensure the currently edited profile uses the latest form data
        // (Just in case newList was stale or something)
        const currentIdx = profilesForServer.findIndex(
        ...
        if (currentIdx >= 0) {
          profilesForServer[currentIdx] = {
            ...profilesForServer[currentIdx],
            ...profileToSave,
          }
        } else {
          profilesForServer.push(profileToSave)
        }

        console.log(
          '📤 Отправляем профили на сервер (Safe Merge):',
          profilesForServer.length,
        )
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      ...
      // Filter server profiles to exclude the deleted one
      // If activeId is not found in server profiles, we just send the existing server profiles (no change),
      // except we want to delete it.
      // Actually, if we want to delete 'activeId', we must filter it out from serverProfiles too.
      const profilesForServer = serverProfiles.filter(
        (p) => p && String(p.id) !== String(activeId),
      )
      ...
      if (filtered.length > 0) {
        ...
        try {
          // If all deleted locally, we should probably also clear server profiles?
          // Or just send profilesForServer (which should be empty if we deleted the last one)
          await authAPI.updateProfiles(profilesForServer)
        } catch (err) {
          console.error('Ошибка при сохранении профилей на сервере:', err)
        }
