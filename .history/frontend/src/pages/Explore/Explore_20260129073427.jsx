...
const Explore = () => {
  ...
  useEffect(() => {
    // Загрузка случайных постов для Explore
    // Логика: все посты отображаются в случайном порядке без фильтров
        ...
        <aside className="explore-sidebar">
          ...
          <nav className="explore-sidebar-nav">
            <Link to="/" className="explore-nav-item">
              <svg className="explore-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {user && (
            <Link to="/profile" className="explore-sidebar-current-user">
              <div className="explore-sidebar-current-avatar">
                {user.avatar && typeof user.avatar === 'string' && user.avatar.startsWith('data:video') ? (
                  <video
                    src={user.avatar}
          ...
          <div className="explore-sidebar-animation">
            <video 
              ...
              playsInline
              onLoadedData={(e) => {
                // Убираем черный переход, устанавливая время начала
                e.target.currentTime = 0.1;
              }}
              onEnded={(e) => {
                // Бесшовный переход - сразу перезапускаем
                e.target.currentTime = 0.1;
                e.target.play();
              }}
            >
            ...
            {/* Анимация мыльных пузырей с аватарками */}
