...
const Explore = () => {
  ...
  useEffect(() => {
    ...
    postsAPI
      .getPostById(explorePostModalId)
      .then((res) => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: 'Post',
          url,
          text: explorePostModalData.caption || '',
        })
      } else {
  ...
  const handleAddComment = async (e) => {
    ...
    if (
      ...
      !exploreModalComment.trim() ||
      exploreModalSubmitting
    )
      return
    setExploreModalSubmitting(true)
    try {
      const res = await postsAPI.addComment(
        explorePostModalData.id,
        exploreModalComment.trim(),
      )
            ...
            {explorePostModalLoading ? (
              ...
              <div className="explore-modal-inner">
                <div className="explore-modal-media">
                  {(() => {
                    const imgs =
                      explorePostModalData.images?.length > 0
                ...
                <div className="explore-modal-side">
                  <div className="explore-modal-actions">
                    ...
                    >
                      <svg
                        className="explore-modal-action-icon"
                        viewBox="0 0 24 24"
                        fill={
                          explorePostModalData.liked ? 'currentColor' : 'none'
                        }
                        stroke="currentColor"
                        strokeWidth="2"
                      >
                      ...
                      <span className="explore-modal-action-count">
                        {formatCount(explorePostModalData.likesCount)}
                      </span>
                    </button>
                    <span className="explore-modal-comments-count">
                      {formatCount(explorePostModalData.commentsCount)}
                    </span>
                    <button
                      type="button"
                      className="explore-modal-action-btn"
                      title="Поделиться"
                    ...
                    >
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill={
                          explorePostModalData.saved ? 'currentColor' : 'none'
                        }
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                      ...
                      <span className="explore-modal-action-count">
                        {formatCount(explorePostModalData.savesCount ?? 0)}
                      </span>
                    </button>
                  </div>
                  <div className="explore-modal-caption-block">
                    <div className="explore-modal-title-text">
                      {explorePostModalData.title || 'Untitled'}
                    </div>
                    <div className="explore-modal-caption">
                      {explorePostModalData.caption || ''}
                    </div>
                  </div>
                    ...
                    {(explorePostModalData.comments || []).map((c) => (
                      <div key={c.id} className="explore-modal-comment">
                        ...
                        <div className="explore-modal-comment-body">
                          <span className="explore-modal-comment-username">
                            {c.user?.username || 'User'}
                          </span>
                          <span className="explore-modal-comment-text">
                            {c.text}
                          </span>
                        </div>
                      </div>
                  ...
                  <form
                    className="explore-modal-add-comment"
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={
                        !exploreModalComment.trim() || exploreModalSubmitting
                      }
                    >
