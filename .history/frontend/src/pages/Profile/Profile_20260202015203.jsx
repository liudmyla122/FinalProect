...
const Profile = () => {
      ...
      try {
        ...
        if (usernameParam && usernameParam.trim()) {
          ...
          try {
            ...
            if (err.response?.status === 404 && isNikiita) {
              ...
              setViewUser(fallbackUser)
              authAPI.getCurrentUser().then((currentRes) => {
        ...
        try {
          ...
          console.log('📡 Ответ сервера getCurrentUser:', {
            ...
            avatarPreview:
              serverUserResponse?.user?.avatar?.substring(0, 50) || 'нет',
          ...
          if (serverUserResponse && serverUserResponse.user) {
            ...
            if (
              ...
              try {
                ...
                console.warn(
                  'Не удалось сохранить user в localStorage:',
                  localError
                )
              }
          ...
          console.error(
            '❌ Ошибка при загрузке аватара из базы данных:',
            serverError
          )
        ...
        if (!avatarToUse) {
          try {
            ...
            console.warn(
              'Ошибка при чтении аватара из отдельного хранилища:',
              storageError
            )
          }
        }
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      // Всегда загружаем все посты и фильтруем на клиенте для надежности
      try {
        ...
        if (response.success) {
          ...
          const filtered = activeProfileId
            ? all.filter((p) => {
                // Если у поста нет profileId, показываем его везде
    ...
    try {
      ...
      if (res.success) {
        const nextLikesCount =
          res.likesCount ??
          (post.likesCount || 0) + (res.liked ? 1 : -1)
        setPosts((prev) =>
          prev.map((p) =>
            p.id === post.id
              ? {
        ...
        if (selectedPost && post.id === selectedPost.id) {
          setSelectedPost((prev) =>
            prev ? { ...prev, liked: res.liked, likesCount: nextLikesCount } : null
          )
        }
      }
    } catch (err) {
              ...
              <div className="profile-details">
                ...
                {viewUser.organization ? (
                  <div className="profile-organization">
                    <span className="profile-organization-label">Organization / company:</span>{' '}
                    <span className="profile-organization-value">{viewUser.organization}</span>
                  </div>
                ) : null}
              </div>
            </div>
  ...
  const postsCount =
    !postsLoading ? posts.length : (user?.postsCount ?? 0)
            ...
            <div className="profile-details">
              ...
              <div className="profile-bio">
                {aboutText ? (
                  ...
                  <>
                    <p>
                      • Гарантия помощи с трудоустройством в ведущие ІТ-компании
                    </p>
                ...
                {user?.organization ? (
                  <div className="profile-organization">
                    <span className="profile-organization-label">Organization / company:</span>{' '}
                    <span className="profile-organization-value">{user.organization}</span>
                  </div>
                ) : null}
              </div>
            </div>
                    ...
                    >
                        ...
                        {miniChatComments.length === 0 ? (
                          ...
                          miniChatComments.map((c) => {
                            ...
                            const isCurrentUser =
                              (user?.dbId || user?.id) &&
                              c.user?.id &&
                              String(c.user.id) === String(user.dbId || user.id)
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={!miniChatNewText.trim() || miniChatLoading}
                        >
                          Send
                        </button>
                      </form>
                    ...
                    <span className="profile-post-modal-action-with-count">
                      <button
                        ...
                        className={`profile-post-modal-icon-btn ${
                          selectedPost.liked ? 'profile-post-modal-icon-btn-active' : ''
                        }`}
                        title="Лайки"
