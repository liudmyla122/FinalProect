...
const Profile = () => {
  ...
  // Убираем дубликаты профилей по id
  ...
  // Форматирование времени как на скриншоте (1 day, 17h, etc.)
  ...
  // При переходе на /profile (без username) сбрасываем чужой профиль — показываем свой
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const initializeProfile = async () => {
      try {
        ...
        // Просмотр чужого профиля по username в URL (/profile/mike)
        if (usernameParam && usernameParam.trim()) {
          try {
            ...
            // Это мой профиль — продолжаем обычную инициализацию
        ...
        // Миграция со старой схемы (только один user)
        ...
        if (!list.length) {
          // ни одного профиля — отправляем на создание
          setLoading(false);
        ...
        if (!activeProfile || !activeProfile.profileCompleted) {
          // Есть активный профиль, но он ещё не заполнен
        ...
        // Загружаем аватар: сначала из базы данных (ВСЕГДА проверяем базу), потом из user, потом из отдельного хранилища, потом из профиля
        ...
        const detectAvatarType = (avatarData) => {
          ...
          // Проверяем MIME тип в base64 строке
          if (avatarData.startsWith('data:video/')) {
            return 'video';
          }
          
          // Проверяем наличие слова video в строке (для надёжности)
          if (avatarData.includes('video') || avatarData.includes('mp4')) {
            return 'video';
          }
          
          // Проверяем размер данных - видео обычно больше изображений
          // Но это не надёжный способ, поэтому используем только как дополнительную проверку
          
          return 'image';
        };
        
        // 1. ВСЕГДА сначала проверяем базу данных (самый надёжный источник)
        try {
          const serverUserResponse = await authAPI.getCurrentUser();
          console.log('📡 Ответ сервера getCurrentUser:', {
            ...
            avatarPreview: serverUserResponse?.user?.avatar?.substring(0, 50) || 'нет',
          ...
          if (serverUserResponse && serverUserResponse.user) {
            ...
            // Проверяем, что аватар не пустая строка и не null/undefined
            if (serverAvatar && 
              ...
              // Определяем тип аватара из базы данных
              // Сначала проверяем, есть ли явно указанный тип в ответе сервера
              ...
              console.log('✅ Аватар загружен из базы данных:', {
                ...
                startsWith: avatarToUse?.substring(0, 30) || 'нет',
                isVideo: avatarToUse?.startsWith('data:video/') || false
              });
              
              // Обновляем user с аватаром, именем/фамилией и id из базы (для сравнения с авторами комментариев)
              ...
              try {
                ...
                console.warn('Не удалось сохранить user в localStorage:', localError);
              }
              
              setProfileAvatarSafe(activeId, avatarToUse);
            } else {
              console.log('⚠️ Аватар не найден в ответе сервера:', {
            ...
            // Если аватара нет, всё равно подставляем имя, фамилию и dbId из базы
            ...
            console.log('⚠️ Некорректный ответ сервера:', serverUserResponse);
          }
        } catch (serverError) {
          console.error('❌ Ошибка при загрузке аватара из базы данных:', serverError);
        ...
        // 2. Если не загрузили из базы, пробуем из localStorage user
        if (!avatarToUse) {
          try {
            const userRaw = localStorage.getItem('user');
            if (userRaw) {
        ...
        // 3. Если не нашли в user, пробуем из отдельного хранилища
        if (!avatarToUse) {
          try {
            ...
            if (avatarFromStorage) {
              ...
              console.log('✅ Аватар загружен из отдельного хранилища');
            }
          } catch (storageError) {
            console.warn('Ошибка при чтении аватара из отдельного хранилища:', storageError);
          }
        }
        
        // 4. Последний вариант - из самого профиля
        if (!avatarToUse && (activeProfile.avatar || activeProfile.profile_image)) {
          ...
          console.log('✅ Аватар загружен из профиля');
        }
        ...
        if (avatarToUse) {
          ...
          // Обновляем объект пользователя, чтобы везде был тот же аватар
        ...
        // Синхронизируем локальное хранилище (list уже без дубликатов)
        localStorage.setItem('profiles', JSON.stringify(list));
        localStorage.setItem('activeProfileId', activeId);
  ...
  // Обновляем профиль при изменении данных в localStorage (например, после редактирования)
  useEffect(() => {
      ...
      if (userRaw && activeIdRaw) {
        try {
          ...
          if (updatedUser.id === activeId && updatedUser.profileCompleted) {
            setUser(updatedUser);
            // Обновляем аватар только если в user он есть — не сбрасываем на null
    ...
    // Также проверяем при фокусе окна (на случай, если данные изменились в другой вкладке)
  ...
  // Догружаем аватар с сервера, если после инициализации его ещё нет (БД — главный источник)
  useEffect(() => {
    if (loading || !user) return;
    const currentAvatar = avatar || user.avatar;
    if (currentAvatar) return; // уже есть
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!cancelled) console.warn('Не удалось догрузить аватар с сервера:', e);
      }
    })();
    return () => { cancelled = true; };
  }, [loading, user?.id, activeProfileId]);

  // Загружаем реальные посты пользователя из backend
  useEffect(() => {
    if (loading) {
      return; // Не загружаем посты, пока идёт инициализация профиля
    }

    const loadPosts = async () => {
      try {
        // Запрашиваем посты только текущего профиля — на бэкенде посты привязаны к profileId
        ...
        console.warn('Ошибка при загрузке постов по profileId, пробуем без фильтра:', error);
      }
      // Fallback: если бэкенд вернул ошибку (например 500) — запрашиваем все посты и фильтруем по profileId на клиенте
      try {
    ...
    // Загружаем только если есть активный профиль
    if (activeProfileId) {
      loadPosts();
    } else {
      setPostsLoading(false);
  ...
  // Загрузка поста с комментариями при открытии модального окна
  useEffect(() => {
    const loadPostWithComments = async () => {
      ...
      try {
        ...
        if (response.success && response.post) {
          ...
          // Обновляем счетчики в selectedPost
  ...
  // При открытии по "Discussion" фокусируем поле ввода комментария
  useEffect(() => {
  ...
  // Загрузка комментариев для мини-чата при открытии по клику на иконку
  useEffect(() => {
    if (!miniChatPostId) {
      setMiniChatComments([]);
      setMiniChatNewText('');
  ...
  /** Полное закрытие модалки поста: крестик или фон — только закрыть, ничего не показывать */
  ...
  const handleAvatarSave = async () => {
    ...
    try {
      ...
      // Отправляем base64-аватар на backend (сохраняется в базу данных)
      ...
      // Обновляем локальное состояние и localStorage по данным из backend
      ...
      setUser(updatedUser);
      setAvatar(savedAvatar); // Используем аватар из ответа сервера
      ...
      // Обновляем профиль в массиве profiles
      try {
        const profilesRaw = localStorage.getItem('profiles');
        if (profilesRaw) {
          ...
          if (activeId) {
            const updatedProfiles = profilesList.map((p) =>
              p.id === activeId
                ? { ...p, avatar: '', avatarType: avatarType } // В localStorage не храним base64 в массиве
                : p
            );
            localStorage.setItem('profiles', JSON.stringify(updatedProfiles));

            // На сервер отправляем аватар текущего профиля, остальных не затираем
            try {
              let serverProfiles = [];
              try {
              ...
              console.warn('Не удалось сохранить профили на сервер:', serverError);
            }
          }
        }
      } catch (profilesError) {
      ...
      // Подгружаем пользователя с сервера, чтобы аватар везде брался из БД
      try {
        ...
        console.warn('Не удалось подгрузить аватар с сервера после сохранения:', e);
      }
      
      // Обновляем отображение аватара сразу (используем сохранённый аватар из сервера)
  ...
  // Просмотр чужого профиля: аватар, имя, подписки, кнопка Follow, посты
  if (viewUser) {
      ...
      <div className="profile-container">
        ...
        <main className="profile-main">
          ...
          <div className="profile-posts-grid profile-posts-empty">
            {postsLoading ? (
              <div className="post-item"><div className="post-image-placeholder">Загрузка постов...</div></div>
            ) : posts.length === 0 ? (
              <div className="post-item"><div className="post-image-placeholder">Нет постов</div></div>
            ) : (
              posts.map((post) => (
  ...
  // Единый источник аватара текущего профиля — используется и в сайдбаре, и в основном блоке, и для постов этого профиля
  ...
  // Аватар профиля по profileId — для постов показываем аватар того профиля, который опубликовал пост
  ...
  const handleEditProfileClick = () => {
    // Редактирование текущего выбранного профиля
    navigate('/profile/edit', { state: { mode: 'edit' } });
  };

  const handleCreateNewProfileClick = () => {
    // Создаём новый пустой профиль и делаем его активным
    const newProfile = {
      id: `profile-${Date.now()}`,
      username: '',
      website: '',
  ...
  const handleProfileSwitch = (profileId) => {
    ...
    // Загружаем аватар выбранного профиля: сначала из localStorage, потом из данных профиля
    let avatarToUse = null;
    try {
    ...
    if (avatarToUse) {
      ...
      setAvatar(null);
      // Если аватара нет локально — подгружаем с сервера (getCurrentUser возвращает аватар пользователя)
      (async () => {
        try {
          const res = await authAPI.getCurrentUser();
          ...
          console.warn('Не удалось подгрузить аватар при переключении профиля:', e);
        }
      })();
    }
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      if (response.success && response.comment) {
        ...
        // Обновляем счётчик комментариев в selectedPost
        setSelectedPost((prev) => ({
          ...prev,
          commentsCount: (prev?.commentsCount || 0) + 1,
        }));
        // Обновляем счётчик комментариев в списке постов
        setPosts((prevPosts) =>
          prevPosts.map((post) =>
  ...
  const handleAddReply = async (commentId, e) => {
    ...
    try {
      ...
      if (response.success && response.comment) {
        // Перезагружаем пост с обновленными комментариями
  ...
  // Переход к сообщениям с выбранным пользователем
  ...
  return (
    <div className="profile-container">
      ...
      <aside className="profile-sidebar">
        ...
        {/* Блок текущего пользователя внизу боковой панели — тот же аватар, что и в профиле (currentProfileAvatar) */}
        {user && (
          <Link to="/profile" className="sidebar-current-user">
            <div className="sidebar-current-avatar">
      ...
      <main className="profile-main">
        ...
        <section className="profile-section">
          <div className="profile-info">
            {/* Аватар — приоритет: state avatar, затем user.avatar из БД/localStorage */}
            <div className="profile-avatar-container">
              <div 
                ...
                {currentProfileAvatar ? (
                  currentProfileAvatarType === 'video' ? (
                    ...
                    <img 
                      ...
                      className="profile-avatar-image"
                      onError={(e) => {
                        console.error('Ошибка при загрузке изображения-аватара:', e);
                      }}
                    />
                  )
                ) : (
            ...
            <div className="profile-details">
              ...
              <div className="profile-bio">
                {aboutText ? (
                  ...
                  <>
                    <p>• Гарантия помощи с трудоустройством в ведущие ІТ-компании</p>
          ...
          {postsLoading ? (
            <div className="profile-posts-grid profile-posts-empty">
              <div className="post-item">
                <div className="post-image-placeholder">Загрузка постов...</div>
              </div>
            </div>
          ) : (
            <div className={`profile-posts-grid ${posts.length === 0 ? 'profile-posts-empty' : ''}`}>
              {/* Плитка с плюсом для добавления нового поста — всегда первая */}
              <button
                type="button"
              ...
              {/* Далее реальные посты пользователя (если они есть) */}
              {posts.map((post) => (
                <div
                  key={post.id}
                ...
                >
                  <div
                    className="post-item"
                    onClick={async () => {
                      // Увеличиваем счетчик просмотров при открытии поста
                      try {
                  ...
                  >
                    {/* Кнопка «три точки» — открывает меню поста */}
                    <button
                      type="button"
                    ...
                    {/* Нижняя зона на карточке: только заголовок поста + статистика */}
                    <div className="post-card-bottom">
                      {(post.title || post.caption) && (
                        <p className="post-item-caption-on-card" title={post.title || post.caption}>
                          <span className="post-item-title-on-card">
                            {post.title || (post.caption && post.caption.length > 70 ? post.caption.slice(0, 70) + '…' : post.caption)}
                          </span>
                        </p>
                      )}
                      <div className="post-stats post-stats-on-card" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="post-stat-btn post-stat-btn-views" title="Просмотры">
                  ...
                  {/* Мини-чат для этого поста */}
                  {miniChatPostId === post.id && (
                    <div className="post-mini-chat" onClick={(e) => e.stopPropagation()}>
                      <div className="post-mini-chat-header">
                        <span>Discussion</span>
                        <button type="button" className="post-mini-chat-close" onClick={() => setMiniChatPostId(null)} aria-label="Закрыть">×</button>
                      </div>
                      <div className="post-mini-chat-list">
                        {miniChatComments.length === 0 ? (
                          <p className="post-mini-chat-empty">Пока нет комментариев</p>
                        ) : (
      ...
      {/* Модальное меню поста (три точки): 1 Редактировать, 2 Посмотреть, 3 Ответить на сообщения, 4 Копировать ссылку, 5 Удалить */}
      {postMenuPostId && (() => {
      ...
      {/* Модальное окно: перенести пост в другой профиль (убрать из текущего) */}
      ...
      {/* Модальное окно просмотра поста — рендерим только при валидном посте, закрытие только через closePostModal */}
          ...
          <div className="profile-post-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="profile-post-modal-content">
              ...
              <div className="profile-post-modal-side">
                {/* Шапка справа (аватар + имя + кнопка подписки) */}
                <div className="profile-post-modal-header">
                  ...
                  {/* В профиле открываются только свои посты — кнопка Follow не нужна */}
                  <span className="profile-post-modal-your-post">Your post</span>
                </div>

                {/* Область комментариев, как на скрине (первый комментарий — твой текст) */}
                <div className="profile-post-modal-comments">
                  {/* Подпись поста как первый комментарий (с режимом редактирования) */}
                        ...
                        <div className="profile-post-modal-comment-header">
                          ...
                          {editingPostId && selectedPost?.id && String(editingPostId) === String(selectedPost.id) ? (
                            <div className="profile-post-modal-edit-caption">
                              <label className="profile-post-modal-edit-caption-label">Заголовок карточки</label>
                              <input
                                type="text"
                                ...
                                maxLength={300}
                              />
                              <label className="profile-post-modal-edit-caption-label">Текст поста</label>
                              <textarea
                            ...
                            {(comment.repliesCount || 0) > 0 && (
                              ...
                              >
                                {expandedReplies.has(comment.id) 
                                  ? 'Скрыть ответы' 
                                  : `Показать ответы (${comment.repliesCount || 0})`}
                              </button>
                            )}
                          </div>
                        </div>
                ...
                {/* Нижняя панель с иконками, лайками и полем комментария */}
                <div className="profile-post-modal-footer">
                  ...
                  <div className="profile-post-modal-stats">
                    <span className="profile-post-modal-likes" title="Лайки">
                      {selectedPost.likesCount || 0}
                    </span>
                    <span className="profile-post-modal-views" title="Просмотры">
                      {selectedPost.viewsCount || 0}
                    </span>
                    <span className="profile-post-modal-comments-count" title="Комментарии">
