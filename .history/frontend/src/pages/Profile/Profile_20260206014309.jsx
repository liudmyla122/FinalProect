    ...
    const loadPosts = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          console.log('📊 Все посты:', all.length)
          console.log('📊 activeProfileId:', activeProfileId)
          const filtered = activeProfileId
            ? all.filter((p) => {
                console.log(`📊 Пост ${p.id}: profileId='${p.profileId}', activeProfileId='${activeProfileId}'`)
                // Если у поста нет profileId, показываем его везде
                if (!p.profileId) {
                  console.log(`✅ Пост ${p.id} показываем (нет profileId)`)
                  return true
                }
                // Сравниваем как строки, чтобы избежать проблем с типами
                const match = String(p.profileId) === String(activeProfileId)
                console.log(`${match ? '✅' : '❌'} Пост ${p.id} ${match ? 'показываем' : 'скрываем'} (profileId match: ${match})`)
                return match
              })
            : all
          console.log('📊 Отфильтрованные посты:', filtered.length)
          setPosts(filtered)
          setPostsLoadError(false)
        }
      } catch (fallbackError) {
