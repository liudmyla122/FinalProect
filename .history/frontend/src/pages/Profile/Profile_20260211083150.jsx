...
const Profile = () => {
        ...
        if (usernameParam && usernameParam.trim()) {
          ...
          try {
            ...
            if (
              ...
              setViewUser(profileUser)
              setUser(currentRes?.user || null)
              // Sync organization with Messages page
              localStorage.setItem(
                'currentViewedOrganization',
                profileUser.organization || ''
              )
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      console.log('🔄 Загружаем посты для профиля:', {
        activeProfileId,
        userId: user?.id,
      ...
      try {
        ...
        if (response.success) {
          ...
          const filtered = loadedPosts.filter((p) => {
            ...
            if (!p.profileId) {
              console.log('📝 Показываем пост без profileId:', p.id)
              return true
            }
          ...
          console.log('📊 Отфильтровано постов:', filtered.length)
