      ...
      try {
        ...
        if (response.success) {
          ...
          const filtered = loadedPosts.filter((p) => {
            ...
            // Проверяем, является ли текущий профиль основным пользователем
            // У основного пользователя могут быть посты с profileId=userId или profileId=null
            const isMainProfile =
              user &&
              (String(activeProfileId) === String(user.dbId) ||
                String(activeProfileId) === String(user.id))

            if (isMainProfile) {
              return (
                !p.profileId || String(p.profileId) === String(activeProfileId)
              )
            }

            // Для дополнительных профилей фильтрация строгая
            // (backend должен вернуть только нужные, но на всякий случай)
            return (
              p.profileId && String(p.profileId) === String(activeProfileId)
            )
          })
        ...
        if (profilesRaw) {
          ...
          if (activeId) {
            const updatedProfiles = profilesList.map((p) =>
              p.id === activeId
                ? { ...p, avatar: savedAvatar, avatarType: avatarType }
                : p,
            )
            localStorage.setItem('profiles', JSON.stringify(updatedProfiles))

            // Мы больше не вызываем updateProfiles здесь, так как updateAvatar на бэкенде
            // теперь умеет обновлять аватар конкретного профиля (через profileId).
            // Это предотвращает риск перезаписи списка профилей устаревшими данными.
          }
        }
      } catch (profilesError) {
