...
export const CreatePostProvider = ({ children }) => {
  ...
  const loadUserWithAvatar = async () => {
    try {
      ...
      console.log(
        '🔄 loadUserWithAvatar - Server response:',
        serverUserResponse
      )
        ...
        if (activeId) {
          if (serverUser.profiles?.length > 0) {
            ...
            if (activeProfile) {
              ...
              console.log(
                '⚠️ Профиль не найден, используем fallback данные'
              )
              if (serverUser.profiles?.length > 0) {
          ...
          console.log('🔄 loadUserWithAvatar - Используем данные пользователя с сервера')
