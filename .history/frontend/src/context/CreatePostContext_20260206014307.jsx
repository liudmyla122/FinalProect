    ...
    try {
      ...
      if (serverUserResponse?.user) {
        ...
        if (activeId && serverUser.profiles?.length > 0) {
          ...
          if (activeProfile) {
            console.log('✅ Найден активный профиль на сервере:', activeProfile.username)
  ...
  const getActiveProfileForModal = () => {
    try {
      ...
      if (userRaw) {
        ...
        if (parsed && (String(parsed.id) === String(activeId) || String(parsed.activeProfileId) === String(activeId))) {
          profile = parsed
      ...
      // Загружаем аватар профиля
      const avatarFromStorage = localStorage.getItem(`profile_avatar_${activeId}`)
      const avatar = avatarFromStorage || profile.avatar || profile.profile_image || ''
      const avatarType = profile.avatarType || (avatar.startsWith('data:video/') ? 'video' : 'image')
      ...
      const profileId = (() => {
        try {
          ...
          if (active?.activeProfileId) {
            console.log('🔄 Создаем пост от имени активного профиля:', active.activeProfileId)
          ...
          console.log('⚠️ Не найден активный профиль, создаем пост без profileId')
          return null
        } catch (e) {
