...
export const CreatePostProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const checkActiveProfile = () => {
      ...
      // Check if the current user state matches the active profile
      const currentUserId = user?.id || user?.dbId || user?._id
      if (String(currentUserId) !== String(activeId)) {
        console.log('🔄 Account switch detected while modal open - updating user data')
        const newProfile = getActiveProfileForModal()
        if (newProfile) {
          setUser(newProfile)
        }
      }
    ...
    window.addEventListener('storage', checkActiveProfile)
    window.addEventListener('user-info-updated', checkActiveProfile)
    window.addEventListener('focus', checkActiveProfile)

    return () => {
      window.removeEventListener('storage', checkActiveProfile)
      window.removeEventListener('user-info-updated', checkActiveProfile)
      window.removeEventListener('focus', checkActiveProfile)
    }
  }, [isOpen, user])
                ...
                <div className="home-create-user-row">
                  <div className="home-create-user-avatar">
                    {(() => {
                      ...
                      const activeId =
                        user?.activeProfileId || localStorage.getItem('activeProfileId')
                      ...
                      let displayAvatar = user?.avatar || user?.profile_image || null
                      ...
                      if (!displayAvatar && activeId) {
                        ...
                        console.log('🎨 MODAL RENDER - Avatar from storage:', !!displayAvatar)
                      }
