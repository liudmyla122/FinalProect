...
export const CreatePostProvider = ({ children }) => {
  ...
  const open = () => {
    ...
    // Загружаем пользователя с сервера — аватар там есть; не перезаписываем потом из localStorage,
    // иначе круг в модалке снова остаётся без картинки
    loadUserWithAvatar().catch(() => {
