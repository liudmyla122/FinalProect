...
export const CreatePostProvider = ({ children }) => {
  ...
  const open = () => {
    ...
    try {
      ...
      console.log('🔄 OPEN MODAL - getActiveProfileForModal returned:', initialUser)
      ...
      if (initialUser) {
        console.log('🔄 OPEN MODAL - Setting user:', initialUser.username, 'avatar:', !!initialUser.avatar)
        setUser(initialUser)
      }
    } catch (e) {
    ...
    console.log('🔄 OPEN MODAL - Setting isOpen=true')
    setIsOpen(true)

    // Асинхронно обновляем данные (включая аватар с сервера/хранилища)
    loadUserWithAvatar()
      .then(() => {
        console.log('🔄 OPEN MODAL - loadUserWithAvatar completed')
            ...
            if (avatarFromStorage) {
              setUser((prev) => {
                if (
                  ...
                  console.log('🔄 OPEN MODAL - Updating avatar from storage')
