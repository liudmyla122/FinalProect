...
export const CreatePostProvider = ({ children }) => {
  ...
  const open = async () => {
    ...
    try {
      ...
      console.log('🔄 OPEN MODAL - getActiveProfileForModal returned:', initialUser?.username)
      ...
      if (initialUser) {
        console.log('🔄 OPEN MODAL - Setting user synchronously:', initialUser.username, 'avatar:', !!initialUser.avatar)
        setUser(initialUser)
      }
    } catch (e) {
      ...
      if (avatarFromStorage) {
        setUser((prev) => {
          if (prev && (!prev.avatar || prev.avatar !== avatarFromStorage)) {
            ...
            return { ...prev, avatar: avatarFromStorage, avatarType: prev.avatarType || (avatarFromStorage.startsWith('data:video/') ? 'video' : 'image') }
          }
          return prev
        })
      }
