  ...
  useEffect(() => {
    const handleStorageUpdate = () => {
      try {
        ...
        // Check active profile to ensure we display the correct one

        // If we have an active profile ID, we should try to find that profile's data
        // because localStorage.user might be the main account, not the selected profile
        if (activeId && parsed) {
          const profilesRaw = localStorage.getItem('profiles')
          if (profilesRaw) {
            const profiles = JSON.parse(profilesRaw)
