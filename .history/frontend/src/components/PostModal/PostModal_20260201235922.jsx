...
export default function PostModal() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setComment('')
    postsAPI.getPostById(postId).then((res) => {
        ...
        {loading ? (
          ...
          <div className="post-modal-inner">
            <div className="post-modal-media">
              {(() => {
                const imgs = postData.images?.length > 0
                  ? postData.images
                  : postData.image
                    ? [postData.image]
                    : []
                const src = imgs[0]
                const isVideo = postData.isVideo ||
            ...
            <div className="post-modal-side">
              <div className="post-modal-actions">
                ...
                >
                  <svg className="post-modal-action-icon" viewBox="0 0 24 24" fill={postData.liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
                    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path>
                  </svg>
                  <span className="post-modal-action-count">{formatCount(postData.likesCount)}</span>
                </button>
                <span className="post-modal-comments-count">{formatCount(postData.commentsCount)}</span>
                <button
                  type="button"
                ...
                >
                  <svg className="post-modal-action-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                >
                  <svg className="post-modal-action-icon" viewBox="0 0 24 24" fill={postData.saved ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path>
                  </svg>
                  <span className="post-modal-action-count">{formatCount(postData.savesCount ?? 0)}</span>
                </button>
              </div>
              <div className="post-modal-caption-block">
                <div className="post-modal-title-text">
                  {postData.title || 'Без названия'}
                </div>
                ...
                {(postData.comments || []).map((c) => (
                  <div key={c.id} className="post-modal-comment">
                    ...
                    <div className="post-modal-comment-body">
                      <span className="post-modal-comment-username">{c.user?.username || 'User'}</span>
