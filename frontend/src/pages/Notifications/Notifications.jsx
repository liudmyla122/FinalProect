...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    setModalComment('')
    postsAPI.getPostById(modalPostId).then((res) => {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const res = await postsAPI.addComment(modalPostData.id, modalComment.trim())
          ...
          {loading ? (
            ...
            <div className="notifications-empty-wrap">
              <div className="notifications-empty">
                ...
                <p>Ставьте лайки постам в ленте — они появятся на этой странице.</p>
                <Link to="/" className="notifications-empty-link">Перейти в ленту</Link>
              </div>
            </div>
          ) : (
      ...
      {modalPostId && (
        <>
          <div
            ...
            aria-hidden="true"
          />
          <div className="notifications-modal" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
            ...
            {modalLoading ? (
              ...
              <div className="notifications-modal-inner">
                <div className="notifications-modal-media">
                  {(() => {
                    const imgs = modalPostData.images?.length > 0
                      ? modalPostData.images
                      : modalPostData.image
                        ? [modalPostData.image]
                        : []
                    const src = imgs[0]
                    const isVideo = modalPostData.isVideo ||
                ...
                <div className="notifications-modal-side">
                  <div className="notifications-modal-actions">
                    ...
                    >
                      <svg className="notifications-modal-action-icon" viewBox="0 0 24 24" fill={modalPostData.liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
                        <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path>
                      </svg>
                      <span className="notifications-modal-action-count">{formatCount(modalPostData.likesCount)}</span>
                    </button>
                    <span className="notifications-modal-comments-count">{formatCount(modalPostData.commentsCount)}</span>
                    <button
                      type="button"
                    ...
                    >
                      <svg className="notifications-modal-action-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    >
                      <svg className="notifications-modal-action-icon" viewBox="0 0 24 24" fill={modalPostData.saved ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path>
                      </svg>
                      <span className="notifications-modal-action-count">{formatCount(modalPostData.savesCount ?? 0)}</span>
                    </button>
                  </div>
                    ...
                    {(modalPostData.comments || []).map((c) => (
                      <div key={c.id} className="notifications-modal-comment">
                        ...
                        <div className="notifications-modal-comment-body">
                          <span className="notifications-modal-comment-username">{c.user?.username || 'User'}</span>
                          <span className="notifications-modal-comment-text">{c.text}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  <form className="notifications-modal-add-comment" onSubmit={handleAddComment}>
                    <input
                      type="text"
