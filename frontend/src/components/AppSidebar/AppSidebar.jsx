...
const AppSidebar = ({ activeItem = 'home' }) => {
      ...
      if (profilesRaw) {
        ...
        if (Array.isArray(profiles) && profiles.length > 0) {
          const activeProfile = activeId
            ? profiles.find((p) => String(p.id) === String(activeId))
            : profiles[0]
  ...
  const nav = (to, key, label, icon, isButton = false) => {
    ...
    return (
      <Link key={key} to={to} className={className} onClick={handleNavClick}>
        {content}
      </Link>
    )
  }
    ...
    <>
      ...
      <aside className={`app-sidebar${isMobileOpen ? ' is-mobile-open' : ''}`}>
        ...
        {user && (
          <Link
            to="/profile"
            className="app-sidebar-current-user"
      ...
      <nav className="app-mobile-nav">
        <Link to="/" className="app-mobile-nav-item">
          <svg
        ...
        <Link to="/explore" className="app-mobile-nav-item">
          <svg
            className="app-mobile-nav-icon"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <Link to="/profile" className="app-mobile-nav-item">
          {sidebarAvatar ? (
            sidebarAvatarIsVideo ? (
              <video
                src={sidebarAvatar}
                className="app-mobile-profile-img"
                autoPlay
                loop
                muted
                playsInline
